...
class CCT:
    ...
    def plot_CCT(self,color_schema,font_schema,ticks_schema,grid_schema,**kwargs):
        ...
        plot_limits = {'X':[1e0,10.0**4.2],'Y':[0.0,1100.0]}
        ...
        #ratio_start_curve = self.Ac3
        for k in range(len(sorted_cooling_rates)):
            start_point_index = 0
            if(k != len(sorted_cooling_rates)):
        ...
        ax.text(plot_limits['X'][0], self.Ac1, ' ' + r'$Ac_1$' + ' = ' + str(self.Ac1) + ' °C', ha='left', va='bottom',color=schema['Ac1']['color'],**lines_labes_fontschema)
        ax.text(plot_limits['X'][0], self.Ac3, ' ' + r'$Ac_3$' + ' = ' + str(self.Ac3) + ' °C', ha='left', va='bottom',color=schema['Ac3']['color'],**lines_labes_fontschema)
        ...
        ax.text(0.8,0.97,r'$T_{aust.}$' + ' = ' + str(int(self.T0)) + ' °C (' + str(T0_F) + ' °F)' + '\nHolding time = 20 min.',zorder=30,ha='left',va='top',transform=ax.transAxes,bbox=dict(facecolor='white', edgecolor='black'),color=schema['T0']['color'],**box_text_fontscheme)
        ...
        for transf in transformations:
            if transf in self.treated_data.keys():
                ...
                if(transf == 'Ms'):
                    ...
                    plt.hlines(self.treated_data['Ms']['Y'][0],plot_limits['X'][0],self.treated_data['Ms']['X'][0],schema[transf]['color'],zorder=10,lw=2.0)
                elif(transf == 'Mf'):
        ...
        #ax.set_title('VP420IM',**font_schema)
...
if __name__=='__main__':
    ...
    second_version_grid = dict(
        ...
        minor=dict(b=False,lw=0.7,ls='-',color='gray',alpha=0.5),
    ...
    '''
    transormation_data = [
        dict(type='Pf',T0=T0,rate=0.8,T=628.0),
        dict(type='Pf',T0=T0,rate=0.4,T=634.0),
        dict(type='Pf',T0=T0,rate=0.2,T=661.0),
        dict(type='Pf',T0=T0,rate=0.1,T=703.0),

        dict(type='Ps',T0=T0,rate=0.8,T=698.0),
        dict(type='Ps',T0=T0,rate=0.4,T=751.0),
        dict(type='Ps',T0=T0,rate=0.2,T=788.0),
        dict(type='Ps',T0=T0,rate=0.1,T=817.0),

        dict(type='C',T0=T0,rate=20.0,T=601.0),
        dict(type='C',T0=T0,rate=12.5,T=692.0),
        #dict(type='C',T0=T0,rate=3.0,T=787.0),
        dict(type='C',T0=T0,rate=1.5,T=820.0),
        #dict(type='C',T0=T0,rate=0.5,T=855.0),
        dict(type='C',T0=T0,rate=0.1,T=888.0),

        dict(type='Ms',T0=T0,rate=50.0,T=248.5),
        dict(type='Ms',T0=T0,rate=10.0,T=260.5),
        dict(type='Ms',T0=T0,rate=1.0,T=287.6),
        dict(type='Ms',T0=T0,rate=0.1,T=338.6),

        dict(type='Mf',T0=T0,rate=50.0,T=92.5),
        dict(type='Mf',T0=T0,rate=10.0,T=92.5),
        dict(type='Mf',T0=T0,rate=1.0,T=92.5),
        dict(type='Mf',T0=T0,rate=0.75,T=92.5),
    ]

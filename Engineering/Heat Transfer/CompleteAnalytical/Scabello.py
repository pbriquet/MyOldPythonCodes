...
if __name__=='__main__':
    ...
    time = np.array(list(np.linspace(1e-4,1.0,num=100)))
    ...
    for x in positions:
        ...
        for t in time:
            ...
            sol[-1].append(test.solution(t))
            mesh[2].append(sol[-1][-1])
        sol[-1] = np.array(sol[-1])
        df['x = ' + '{:1.1f}'.format(x)] = sol[-1]
    
    for t in time:
        ave.append(test.average(t))
    
    ave = np.array(ave); sol = np.array(sol)
    df['Average'] = ave
    df['t'] = time
    ...
    if(plotter != 'plot_contour'):
        for k,v in enumerate(positions):
            ax.plot(tau*time,100.0*sol[k],color=color[k],label='x = ' + "{:10.2f}".format(v/2.0*body['a']*1e3))
        ax.plot(tau*time,100.0*ave,color='black',linestyle='--',label='Average')
        ax.set_xlabel('t (h)')
        ax.set_ylabel('% H')
        ax.set_xlim(time[0]*tau,time[-1]*tau)
        ax.set_ylim(0.0,100.0)
        plt.legend()
        plt.show()
    elif(plotter == 'plot_contour'):
        ...
        if(dimensionless_time):
            ax.set_xlabel(r'$t^*=t/\tau$' + '\t' + r'$\tau=a^2/D=$' + '{:1.1f}'.format(tau) + ' h')
        else:
            ax.set_xlabel(r'$t (h)$')

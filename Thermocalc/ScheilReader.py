...
class ScheilReader:
    ...
    def plot(self):
        ...
        #ax.set_yscale('log')
        ax.set_xlabel(r'$\epsilon_s$ = solid fraction (-)')
...
if __name__=='__main__':
    __location__ = os.path.realpath(os.path.join(os.getcwd(), os.path.dirname(__file__)))
    folderpath = os.path.join(__location__,'Bonon\\Scheil')
    file0 = os.path.join(folderpath,'scheil.exp')
    e = ScheilReader(file0,verbose=False)
    #names = [('BPW(LIQUID)','L'),('BPW(FCC_A1#1)',r'$\gamma$'),('BPW(BCC_A2)',r'$\alpha$'),('BPW(M6C)',r'$W_6C$'),('BPW(MNS)',r'$MnS$'),('BPW(FCC_A1#2)',r'$NbC$'),('BPW(FCC_A1#3)',r'$VC$'),('BPW(M23C6)',r'$Cr_{23}C_6$')]
    #e.phases_plot_names(names)
    e.plot()
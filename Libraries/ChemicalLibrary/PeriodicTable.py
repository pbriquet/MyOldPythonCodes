...
class Alloy:
    def __init__(self,percent='wt',balance='Fe',**kwargs):
        p = PeriodicTable()
        self._elements = dict()
        self._xelements = dict()
        sum_percent = 0.0
        N = 0.0
        for k,wt in kwargs.items():
            self._elements[k] = wt
            sum_percent += wt
            N += wt/p.elements[k].atomicweigth
        self._elements[balance] = 100.0 - sum_percent
        sum_percent = 0.0
        for k,wt in kwargs.items():
            self._xelements[k] = wt/p.elements[k].atomicweigth/N
            sum_percent += self._xelements[k]
        self._xelements[balance] = 100.0 - sum_percent
    def get_wt(self):
        return self._elements
    def get_at(self):
        return self._xelements
    def __getitem__(self,arg):
        if(self._elements.has_key(arg)):
            return self._elements[arg]
        else:
            return 0.0
